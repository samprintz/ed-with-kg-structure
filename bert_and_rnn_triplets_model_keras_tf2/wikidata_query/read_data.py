...
def convert_text_into_vector_sequence(model, text, max_sentence_length):
...
def get_item_mask_for_words(text, item, max_sentence_length):
    ...
    # NEW Padding
    while len(types) < max_sentence_length:
        types.append(np.zeros(embedding_size))
    return types
...
def create_text_item_graph_dict(text, item, wikidata_id, max_sentence_length):
    text_item_graph_dict = {}
    text_item_graph_dict['text'] = text
    text_item_graph_dict['item'] = item
    text_item_graph_dict['wikidata_id'] = wikidata_id
...
def get_json_data(json_data):
    ...
    for json_item in json_data:
        try:
            ...
            text_item_graph_dict = create_text_item_graph_dict(text, item, wikidata_id, max_sentence_length)
            text_item_graph_dict['answer'] = _is_relevant
            data.append(text_item_graph_dict)
...
def get_max_sentence_length(json_data):
    max_sentence_length = 0
    for json_item in json_data:
        sentence_length = len(json_item['text'])
