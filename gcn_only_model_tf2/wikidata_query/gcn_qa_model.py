...
tf.compat.v1.disable_eager_execution() # Fix 10.03.2021
...
def GCN_layer_fw(embedding_size, hidden_layer1_size, memory_dim, hidden, Atilde_fw, question_vector, name):
    W0_fw = tf.Variable(tf.random.uniform([embedding_size + memory_dim, hidden_layer1_size], -_rw, _rw),
                        name='W0_fw' + name)
    b0_fw = tf.Variable(tf.random.uniform([hidden_layer1_size], -_rw, _rw), name='b0_fw' + name)
    left_X1_projection_fw = lambda x: tf.matmul(tf.concat([x, question_vector], axis=1), W0_fw) + b0_fw
...
def GCN_layer_bw(embedding_size, hidden_layer1_size, memory_dim, hidden, Atilde_bw, question_vector, name):
    W0_bw = tf.Variable(tf.random.uniform([embedding_size + memory_dim, hidden_layer1_size], -_rw, _rw),
                        name='W0_bw' + name)
    b0_bw = tf.Variable(tf.random.uniform([hidden_layer1_size], -_rw, _rw), name='b0_bw' + name)
    left_X1_projection_bw = lambda x: tf.matmul(tf.concat([x, question_vector], axis=1), W0_bw) + b0_bw
    left_X1_bw = tf.map_fn(left_X1_projection_bw, hidden)
    left_X1_bw = tf.transpose(a=left_X1_bw, perm=[1, 0, 2], name='left_X1_bw' + name)
    X1_bw = tf.nn.relu(tf.matmul(Atilde_bw, left_X1_bw))
    X1_bw = tf.transpose(a=X1_bw, perm=[1, 0, 2])
    return X1_bw
